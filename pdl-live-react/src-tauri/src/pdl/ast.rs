...
impl SequencingBlock for LastOfBlock {
    ...
    fn to_block(&self) -> PdlBlock {
        PdlBlock::Advanced(PdlAdvancedBlock::LastOf(self.clone()))
    }
    fn result_for(&self, output_results: Vec<PdlResult>) -> PdlResult {
...
impl SequencingBlock for TextBlock {
    ...
    fn to_block(&self) -> PdlBlock {
        PdlBlock::Advanced(PdlAdvancedBlock::Text(self.clone()))
    }
    fn result_for(&self, output_results: Vec<PdlResult>) -> PdlResult {
        PdlResult::String(
            output_results
...
/// A PDL program/sub-program consists of either a literal (string, number, boolean) or some kind of structured/"Advanced" block
#[derive(Serialize, Deserialize, Debug, Clone)]
#[serde(untagged)]
pub enum PdlBlock {
    Bool(bool),
    Number(Number),
    String(String),
    Advanced(PdlAdvancedBlock),
}

/// Some kind of non-literal block, i.e. a block with some structure
#[derive(Serialize, Deserialize, Debug, Clone)]
#[serde(untagged)]
pub enum PdlAdvancedBlock {
    If(IfBlock),
    Import(ImportBlock),
