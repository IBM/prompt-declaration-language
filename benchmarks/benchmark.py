...
class Benchmark_Base(ABC):
    ...
    def run(self):
        ...
        with open(self.config.dataset, 'r') as dataset_f, open(self.config.examples_results_path, 'w') as examples_results_file, ThreadPoolExecutor(max_workers=n_workers) as executor:
            try:
                for line in dataset_f:
                    ...
                    try:
                        ...
                        console.log(f"Fail to solve example {example_id}: {exc}\n{err_msg}")
                    ...
                    aggregated_results.llm_usage.completion_tokens += llm_usage.completion_tokens
                    aggregated_results.llm_usage.prompt_tokens += llm_usage.prompt_tokens
                    console.log(f"example {example_id}: {llm_usage.model_calls:3d} model calls  {llm_usage.prompt_tokens:5d} prompt tokens  {llm_usage.completion_tokens:5d} completion tokens  {solve_time} s")
                    ...
                    if solutions is None:
                        ...
                        for solution in solutions.values:
                            passes_futures.append(executor.submit(self.passes, solution, datapoint))
                        for i, (solution, score, passes_future) in enumerate(zip(solutions.values, solutions.logits, passes_futures)):
                        ...
                        console.log(f"Pass@1: {pass_at_1_passes} (solutions: {pass_at_1_solution})")

                        # Majority voting
                        most_frequent_solution, most_frequent_passes = _get_most_frequent(results)
                        if most_frequent_passes:
                            aggregated_results.maj.successes += 1
                        else:
                            aggregated_results.maj.failures.append(example_id)
                        console.log(f"Majority voting: {most_frequent_passes} (solutions: {most_frequent_solution})")
                        ...
                        for i, m in enumerate(solutions.metadata):
                            if len(m) != 1:
                                print(f"XXXXX Example {example_id}: the number of metadata for solution {i} is not 1. This should not arrive!\n{m}", flush=True, file=stderr)
                    ...
                    print(example_result.model_dump_json(), file=examples_results_file, flush=True)
                    
                    console.log("**** Accuracy:")
                    console.log(f"  pass@1: {show_stats(aggregated_results.pass_at_1, aggregated_results.total)}")
                    console.log(f"  pass@k: {show_stats(aggregated_results.pass_at_k, aggregated_results.total)}")
                    console.log(f"  maj:    {show_stats(aggregated_results.maj, aggregated_results.total)}")
                    console.log(f"  mode:   {show_stats(aggregated_results.mode, aggregated_results.total)}")
                    console.log(f"  success expectation: { sum(success_probabilities) / aggregated_results.total}")
                    console.log(f"**** Total: {aggregated_results.total}")
            finally:
                success_expectation = sum(success_probabilities) / aggregated_results.total
                ...
                with open(self.config.aggregated_results_path, "w") as f:
...
def _get_most_frequent(results):
    ...
    for i, solution, _, passes in results:
        if solution in counts:
            ...
            if b != passes:
                print(f"XXXXXXXXXXXXXXXXXXXX THIS SHOULD NOT APPEND: the same solutions {i} and {ids} have different pass status:\n```\n{solution}```", file=stderr, flush=True)
            ids = ids + [i]
            n = n + 1
            b = b or passes
        else:
    ...
    most_frequents = [ (ids, passes) for _, (ids, n, passes) in counts.items() if n == max_frequency ]
...
def make_results_paths(config_name, results_dir):
    ...
    aggregated_results_path = f"{results_dir}/{config_name}_{time_stamp}_aggregated.json"
