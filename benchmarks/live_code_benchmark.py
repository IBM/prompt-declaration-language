...
class Live_Code_Base(Benchmark_Base):
    ...
    def passes(self, solution, datapoint) -> bool:
        ...
        try:
            ...
            print(f"True: { is_list_of_true(([True, True, True, True, True], {'execution time': 0.004155635833740234}))}")
            print(f"False: { is_list_of_true(([-2], {'output': 'YES\nYES\nYES\nYES\nYES\nYES\n', 'inputs': '6\nabc\nacb\nbac\nbca\ncab\ncba\n', 'expected': 'YES\nYES\nYES\nNO\nNO\nYES\n', 'error_code': -2, 'error_message': 'Wrong answer at output_line_idx=3: YES != NO'}))}")
...
class Live_Code_PPDL(Live_Code_Base):
    def solve(self, problem: str):
        config = PpdlConfig(
