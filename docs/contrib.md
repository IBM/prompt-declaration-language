...
Concretely, the model is run greedily (`temperature: 0`, `seed: 0`) and its output is reproducible byte for byte on a given runner — repeated calls and Ollama restarts all agree. It differs between runners because llama.cpp selects its SIMD kernels from the CPU feature flags, and the resulting rounding differences flip the occasional token, after which the rest of the text diverges. So the accepted variants of a result correspond to the CPU generations in the GitHub runner pool: a handful per example, not an open-ended list. The Python version has no effect on model output — 3.11, 3.12 and 3.14 produce byte-identical text on the same machine — so the three matrix jobs of a nightly are three independent draws from that pool, which is why a nightly hits an unrecorded CPU class more often than a single run does. The Ollama version and the model digest are pinned in `.github/actions/ollama/action.yml` for the same reason — changing either invalidates every recorded result, so bump them deliberately and regenerate.
