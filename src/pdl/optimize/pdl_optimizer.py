...
class PDLOptimizer:
        ...
        while len(candidates) < num_candidates:
            ...
            if demo_indices is None:
                demo_indices_used = self.sample_random_indices(
                    ...
                    size=num_demonstrations,  # self.num_demonstrations,
                )
            else:
    ...
    def run(self):
        ...
        # candidates = self.sample_candidates(max_candidates, demo_indices=list(range(self.num_demonstrations)))  # sample
        # for candidate in candidates:
        #     logger.info(f"Candidate indices: {candidate['demonstrations_indices']}")
        scores = []
        ...
        starting_validation_set_size = min(
            self.starting_validation_set_size, validation_set_size
        )
        ...
        table.add_row(
            "Starting validation set size", f"{starting_validation_set_size:,}"
        )
        ...
        if self.test_set_name != self.validation_set_name:
            ...
            # gsm8k problematic: [25, 32, 37, 38, 54, 66, 63]
            range_end = min(
                ending_validation_set_size, len(self.dataset[self.test_set_name])
            )
            ...
            # logger.warning("Failed indices:", final_results)
    ...
    def evaluate(
        ...
        for index, example in enumerate(test_set):
            if (
                ...
                # logger.info(f"Cache hit! {index} {candidate['uuid']}")
                self.pbar.update(1)
                continue
