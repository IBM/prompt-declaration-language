...
def rag_index(input: list[str],
              encoder_model: str, embed_dimension: int,
    ...
    client = MilvusClient(database_name)  # Use URL if talking to remote Milvus (non-Lite)
    ...
    id = 0 # There is also an auto-id feature in Milvus, which we are not using
    for text in input:
        vector = response.data[id]["embedding"]
        res = client.insert(
            collection_name=collection_name,
            data=[{
                "id": id,
...
def rag_retrieve(input: str,
                 encoder_model: str,
                 limit: int,
                 database_name: str, collection_name: str) -> str:
